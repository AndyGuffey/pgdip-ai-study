# Constrained generation / decoding controls: runs the same prompt through a
# local gpt2 model twice — once with sampling and a higher token budget
# (unconstrained, more varied/rambling output) and once with greedy decoding
# and a tight token cap (constrained, short and deterministic) — to show how
# generation parameters (max_new_tokens, do_sample, temperature) shape
# output. The commented-out section below sketches further temperature-
# control and hard length-constraint experiments.
...
print(f"❌ Unconstrained: '{unconstrained[0]['generated_text']}'")

constrained = gen(prompt, max_new_tokens=2, do_sample=False)
print(f"✅ Constrained:   '{constrained[0]['generated_text']}'")
