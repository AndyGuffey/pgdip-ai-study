# Sends the same prompt to OpenAI's gpt-4o-mini model three times using
# streaming responses, measuring time-to-first-token (TTFT), total time,
# and tokens/sec for each run to check whether responses get faster on
# repeat calls.
# Learning purpose: understand streaming-based latency metrics (TTFT vs.
# total time) and empirically test (and explain) OpenAI's lack of
# response caching between independent API calls.
