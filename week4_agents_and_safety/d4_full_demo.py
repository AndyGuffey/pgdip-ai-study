# Full week 4 day 4 demo: combines OpenAI function/tool calling with agent
# observability/tracing in one script. An `add` tool is defined and offered
# to the model; the model decides whether to call it; the tool executes
# locally and returns a result; and every step along the way (the user
# message, the model's decision, and each tool call/result) is logged as a
# JSON-line trace event via a local `log_event` helper.
