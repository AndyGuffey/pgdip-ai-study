# Minimal agent example: an LLM plans which tool(s) to use for a query,
# the code acts on that plan by calling a (mocked) tool, then checks the
# result before returning a final answer. Demonstrates the basic
# plan -> act -> check agent loop with a single tool (get_weather).
