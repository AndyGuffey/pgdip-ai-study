# Structured output validation & repair: validates a flawed, LLM-style JSON
# product record against a strict JSON Schema (catching type mismatches,
# disallowed enum values, and extra items), then attempts to auto-correct
# the common mistakes (string price with a currency symbol, non-standard
# currency code, inconsistent casing, non-string tags) and re-validates,
# showing the before/after diagnostics either way.
...
try:
    ...
    print("❌ JSON syntax error:", e)  # Handle malformed JSON
    exit(1)
